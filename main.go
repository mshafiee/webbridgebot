...
func (b *TelegramBot) handlePlayer2(w http.ResponseWriter, r *http.Request) {
	...
	var lastURLs []FileMeta
	for _, meta := range b.urlHistory[chatID] {
		lastURLs = append(lastURLs, meta)
	}
	...
	tmpl := `
        ...
        <html>
        <head>
            <title>Last URLs</title>
        </head>
        <body>
            <h1>Last URLs for Chat ID: {{.ChatID}}</h1>
            {{range .LastURLs}}
                <p><a href="{{.}}">{{.}}</a></p>
            {{end}}
        </body>
	...
	t, err := template.New("last_urls").Parse(tmpl)
	if err != nil {
	...
	data := struct {
		ChatID   int64
		LastURLs []template.URL
	}{
		ChatID:   chatID,
		LastURLs: make([]template.URL, len(lastURLs)),
	}

	for i, url := range lastURLs {
		data.LastURLs[i] = template.URL(url.URL)
	}
